  ...
  return (
    <main className="console-shell">
      ...
      <section className="main-column">
        ...
        {activeSection === 'webhooks' ? <WebhooksPanel selectedWorkspaceId={selectedWorkspaceId} webhookForm={webhookForm} setWebhookForm={setWebhookForm} webhooks={webhooks} onSubmit={handleWebhookCreate} /> : null}
...
function WebhooksPanel({ selectedWorkspaceId, webhookForm, setWebhookForm, webhooks, onSubmit }: { selectedWorkspaceId: string; webhookForm: { name: string; url: string; secret: string }; setWebhookForm: Dispatch<SetStateAction<{ name: string; url: string; secret: string }>>; webhooks: Webhook[]; onSubmit: (event: FormEvent<HTMLFormElement>) => Promise<void>; }) {
  return <section className="panel glass-panel"><SectionHeading title="Webhooks" subtitle="Buat endpoint callback tenant yang akan dipakai worker webhook." /><form className="stack-form compact" onSubmit={onSubmit}><label className="field-block"><span>Nama</span><input value={webhookForm.name} onChange={(event) => setWebhookForm((current) => ({ ...current, name: event.target.value }))} placeholder="Main webhook" required /></label><label className="field-block"><span>URL</span><input value={webhookForm.url} onChange={(event) => setWebhookForm((current) => ({ ...current, url: event.target.value }))} placeholder="https://example.com/webhook" required /></label><label className="field-block"><span>Secret</span><input value={webhookForm.secret} onChange={(event) => setWebhookForm((current) => ({ ...current, secret: event.target.value }))} placeholder="supersecretkey" required /></label><button className="button-secondary" disabled={!selectedWorkspaceId} type="submit">Tambah Webhook</button></form><div className="list-stack compact-list">{webhooks.length ? webhooks.map((hook) => <article className="card-row slim" key={hook.id}><div><strong>{hook.name}</strong><p>{hook.url}</p><small>{hook.isActive ? 'Aktif' : 'Nonaktif'} · {formatDateTime(hook.createdAt)}</small></div></article>) : <p className="empty-copy">Belum ada webhook aktif.</p>}</div></section>;
}
